...
PID_FILE = "/root/Echelon/valentrix/mm/config/pid/thea.pid"
LOG_FILE = "/root/Echelon/valentrix/mm/logs/thea.log"
...
def aggregate_orderbook(bids: List, asks: List) -> Dict:
    ...
    bid_total = sum(safe_float(b[1]) for b in bids[:BOOK_DEPTH])
    ask_total = sum(safe_float(a[1]) for a in asks[:BOOK_DEPTH])
...
def check_symbol_maturity(client: KucoinClient, symbol: str) -> bool:
    """Check if symbol has been trading for at least MIN_COIN_AGE days."""
    try:
        # Get klines to check history
        end_time = int(time.time())
        start_time = end_time - (MIN_COIN_AGE * 24 * 3600)
...
class SignalsCollector:
        ...
        try:
            ...
            try:
                candles_15m = self.client.get_klines(
                    symbol=symbol,
                    interval='15min',
                    limit=CANDLE_15M_LIMIT
                ).get('data', [])

                candles_1h = self.client.get_klines(
                    symbol=symbol,
                    interval='1hour',
                    limit=CANDLE_1H_LIMIT
                ).get('data', [])
            ...
            # Get orderbook
            try:
                ob_resp = self.client.get_order_book(symbol, depth=BOOK_DEPTH)
    ...
    def run(self):
        ...
        logger.info(f"[CONFIG] Capture interval: {PRC_THEA}s")
        logger.info(f"[CONFIG] Volume range: {MIN_24H_VOLUME:,} - {MAX_24H_VOLUME:,} USDT")
        logger.info(f"[CONFIG] Minimum maturity: {MIN_COIN_AGE} days")
        ...
        while not shutdown_requested:
            try:
                ...
                # Update heartbeat
                if time.time() - self.last_heartbeat >= HB_THEA:
                    update_heartbeat("thea")
                    self.last_heartbeat = time.time()
                
                # Sleep until next interval
                elapsed = time.time() - start_time
                sleep_time = max(PRC_THEA - elapsed, 1)

...
def drop_old_partitions(cur) -> int:
    ...
    logger.warning(f"🜁 Set partitions to {partitions}")
...
def drop_old_signals_partitions(cur) -> int:
    ...
    logger.warning(f"[TIMING] Partition query completed in {elapsed:.3f}s")
    
    for partition_name, partition_range in partitions:
        try:
            to_str = partition_range.split('TO (')[1].split(')')[0].strip("'")
            logger.error(f"-> to string {to_str}")
            to_ts = int(to_str)
            logger.error(f"-> to string as an integer {to_ts}")
            to_datetime = datetime.utcfromtimestamp(to_ts)
            logger.error(f"-> datetime with integer {to_datetime}")
    ...
    logger.warning("[RETURN] drop_old_signals_partitions is returning.")
    return dropped_count
    logger.warning(f"[DONE] drop_old_signals_partitions returned {dropped_count}")

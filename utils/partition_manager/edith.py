...
def create_signals_partitions(cur, hours_ahead: int = 3) -> int:
    ...
    for h in range(1, hours_ahead + 1):
        ...
        start_ts = int(target_hour.timestamp())
        logger.warning(f"Start time set to -> {start_ts}")
        end_ts = int(hour_after.timestamp())
        logger.warning(f"End time set to -> {end_ts}")
        
        partition_name = f"signals_intel_{target_hour.strftime('%Y_%m_%d_%H')}"
        logger.warning(f"Partition name set to -> {partition_name}")
        
        try:
            logger.warning(f"try->Start time set to -> {start_ts}")
            logger.warning(f"try->End time set to -> {end_ts}")
            cur.execute(f"""
                CREATE TABLE IF NOT EXISTS {partition_name}

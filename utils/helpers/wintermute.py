...
def init_logging(LOG_SELF=True, LOG_MAIN=True, SCREEN_OUT=True, LOGGER="Julius"):
    ...
    # 🔹 Initialize for use in this file ===========================
...
def count_open_orders(symbol: str, conn) -> int:
    ...
    try:
        ...
        logger.error(f"Error counting open orders for {symbol}: {e}")
        return 0
        ...
        if backup_path.exists():
            try:
                with open(backup_path, 'r') as f:
                    state = json.load(f)
                logger.info(f"State loaded from backup")
                return state
            except:
                pass
        return {}
...
def write_pid_file(filepath: str) -> None:
    ...
    try:
        ...
        logger.info(f"PID {os.getpid()} written to {filepath}")
    except Exception as e:
...
def cleanup_pid_file(filepath: str) -> None:
    ...
    try:
        if os.path.exists(filepath):
            os.remove(filepath)
            logger.info(f"PID file {filepath} removed")
    except Exception as e:

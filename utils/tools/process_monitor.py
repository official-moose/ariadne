...
def parse_partition_info(partition_name):
    ...
    try:
        print(f"Debug: Parsing partition {partition_name}")
        
        # Extract date/time from partition name
        parts = partition_name.split('_')
        
        # Handle both tickstick (5 parts) and signals_intel (6 parts)
        if len(parts) == 5:
            # tickstick_YYYY_MM_DD_HH
            year, month, day, hour = int(parts[1]), int(parts[2]), int(parts[3]), int(parts[4])
        elif len(parts) == 6:
            # signals_intel_YYYY_MM_DD_HH (take last 4 parts)
            year, month, day, hour = int(parts[2]), int(parts[3]), int(parts[4]), int(parts[5])
        else:
            # Unknown format
            return "Unknown", "Unknown", "Unknown", RESET
        
        print(f"Debug: Year={year}, Month={month}, Day={day}, Hour={hour}")
        ...
        print(f"Debug: Date={date_str}, Time Range={time_str}, Status={status}")
        
        return date_str, time_str, status, color
        
    except Exception as e:
        print(f"Debug: Error parsing {partition_name}: {e}")
    ...
    while True:
        ...
        try:
            ...
            for row in cur.fetchall():
                ...
                process_lower = process.lower().replace("hb_", "")
                cycle_time = "-"
            ...
            if partitions:
                ...
                for (partition,) in partitions:
                    print(f"Debug: Processing partition {partition}")
                    date_str, time_str, status, color = parse_partition_info(partition)
                    print(f"{color}{partition:<28} {date_str:<15} {time_str:<15} {status:<10}{RESET}") 
            else:
                print("No partitions found")
